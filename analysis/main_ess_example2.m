% ess reactive modulation example
%
% Purpose: Example of how to use ess.m to modulate the reactive current
...
%       mess_sig_original.m. Then move mess_sig_example2.m into /pstess
...
save('../analysis/mat/ess_example2_nonlinear_sim','t','bus_v','g');
...
save('../analysis/mat/ess_example2_linear','a_mat','b_ess_p','b_ess_q','c_v','c_ang');

%% Simulate linear model

tL = 0:t_step:t_end;

u = zeros(size(b_ess_q,2),length(tL));
...
load('../analysis/mat/ess_example2_nonlinear_sim','t','bus_v');
y_v = y_v + abs(bus_v(:,1))*ones(1,length(tL));
y_ang = y_ang + angle(bus_v(:,1))*ones(1,length(tL));
bus_vL = transpose(y_v.*exp(1j*y_ang)).';
save('../analysis/mat/ess_example2_linear_sim','tL','bus_vL','u');

%% Plot Nonlinear vs Linearized
clear all; close all; clc;
cd('../analysis');
load('./mat/ess_example2_nonlinear_sim','t','bus_v','g');
load('./mat/ess_example2_linear_sim','tL','bus_vL','u');
...
q_ord = imag(g.ess.ess_scmd + g.ess.ess_sinj(:,1) + g.ess.ess_dsig);
q_ord = u + resample(q_ord.',t,1/t_step).';
...
legend(ax{2}(1),'nonlinear','linearized','location','southeast');
legend(ax{2}(2),'nonlinear','linearized','location','southeast');
...
plot(ax{3}(1),tL,q_ord(1,:),'b',t,imag(g.ess.ess_sinj(1,:)),'m:');
plot(ax{3}(2),tL,q_ord(3,:),'b',t,imag(g.ess.ess_sinj(3,:)),'m:');
...
print(fig{1},'-dpng','-r600','./fig/ess_example2_voltage_mag.png');
print(fig{2},'-dpng','-r600','./fig/ess_example2_bus_freq.png');
print(fig{3},'-dpng','-r600','./fig/ess_example2_pwr_cmd.png');
