...
if ~isempty(g.ess.ess_con)
    if (flag == 0)  % initialization
        if (i ~= 0)
            ...
            error('ess: initialization must be vectorized.');
        else
    ...
    if (flag == 2 || flag == 3)  % dynamics calculation
        if (i ~= 0)
            % caution: non-vectorized computation is not supported!
            error('ess: dynamics calculation must be vectorized.');
        else
            ...
            p_ord_lb(g.ess.ess_soc(:,k) + 1e-6 >= g.ess.ess_con(:,13)) = 0.0;
            p_ord_ub(g.ess.ess_soc(:,k) - 1e-6 <= g.ess.ess_con(:,12)) = 0.0;
    ...
    if (flag == 4)  % current calculation for nc_load
        if (i ~= 0)
            % caution: non-vectorized computation is not supported!
            error('ess: network interface calculation must be vectorized.');
        else
