% y_switch
%
% Purpose: script file for calculating the reduced Y matrices
...
% Author:  Ryan Elliott
% Date:    August 2019
% Purpose: Modified to add option 8, a resistive dynamic brake insertion
%
% Version: 1.1
% Author:  Graham Rogers
% Purpose: modified to add option 7, clear fault with out loss of line
%          suggested by Trong Nguyen and M.A. Pai University of Illinois
%          errors in line to line, line to ground and line to line to ground
%          corrected
%
% Version: 1.0
% Author:  Graham Rogers
...
if (f_type < 4 || f_type == 7 || f_type == 8)
    f_nearbus = g.sys.sw_con(2,2);
    bus_idx = find(bus_f(:,1) == f_nearbus);
    if isempty(bus_idx)
        error('y_switch: faulted bus not specified correctly.');
    end
    ...
    elseif (f_type == 1)
        ...
        xf = g.sys.sw_con(2,4)*g.sys.sw_con(2,5)/(g.sys.sw_con(2,4) ...
                                                  + g.sys.sw_con(2,5));
        xf = max(xf,1e-7);
        bf = 1.0/xf;
    elseif (f_type == 2)
    ...
    elseif (f_type == 3)
        ...
        bf = 1.0/xf;
    elseif (f_type == 8)
        % three-phase resistive shunt insertion
        rf = g.sys.sw_con(2,4);
        rf = max(rf,1e-7);
        gf = 1.0/rf;
        bf = 0.0;
    ...
    if exist('gf','var')
        bus_f(bus_idx(1),8) = gf;
    end
end

% loss of line with no fault
if (f_type == 4)
    ...
    if ~isempty(line_idx)
        ...
        error('y_switch: cannot find faulted line in line data.');
    end
end
...
if (f_type == 5)
    ...
    if isempty(bus_idx)
        error('y_switch: cannot find faulted bus in bus data.');
    end
...
[Y_gf,Y_gncf,Y_ncgf,Y_ncf,V_rgf,V_rncf,bof] = red_ybus(bus_f,line_f);
bus_intf = g.bus.bus_int;
...
if (f_type < 4)
    ...
    if isempty(line_idx)
        estr = '\ny_switch: cannot find branch between buses ';
        estr = [estr, '%0.0f and %0.0f in line data.'];
        error(sprintf(estr,[f_nearbus,f_farbus].'));
    end
    ...
    [Y_gpf1,Y_gncpf1,Y_ncgpf1,Y_ncpf1,V_rgpf1,V_rncpf1,bopf1] = ...
        red_ybus(bus_pf1,line_pf1);     % post-fault admittance matrix

    bus_intpf1 = g.bus.bus_int;
...
if (f_type < 4)
    ...
    [Y_gpf2,Y_gncpf2,Y_ncgpf2,Y_ncpf2,V_rgpf2,V_rncpf2,bopf2] = ...
        red_ybus(bus_pf2,line_pf2);     % post-fault admittance matrix

    bus_intpf2 = g.bus.bus_int;
else
