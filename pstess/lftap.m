...
if (~isempty(max_v_idx) | ~isempty(min_v_idx))
    ...
    if ~isempty(max_v_idx)
        ...
        for fb = 1:length(max_v_idx)
            ...
            if ~isempty(chk_fb)
                % freeze dc taps
                if ~isempty(g.dc.n_dcl)
                    for kt = 1:2*g.dc.n_dcl
                        dc_chk = find(chk_fb == g.dc.ac_line(kt));
                        if ~isempty(dc_chk)
                            chk_fb(dc_chk) = [];
                        end
                    end
            ...
            if ~isempty(chk_fb)
                if (line(chk_fb,8) ~= 0)
                    ...
                    wstr = '\nlftap: voltage high, changing tap on line %0.0f.';
                    warning(sprintf(wstr,chk_fb));
                    ...
                    wstr = '\nlftap: tap reset to %0.0f.';
                    warning(sprintf(wstr,tap));
                    line(chk_fb,6) = tap;
                end
            end
        end
    ...
    if ~isempty(min_v_idx)
        ...
        for fb = 1:length(min_v_idx)
            ...
            if ~isempty(chk_fb)
                if (line(chk_fb,8) ~= 0)
                    % can change tap
                    wstr = '\nlftap: voltage low, changing tap on line %0.0f.';
                    warning(sprintf(wstr,chk_fb));
