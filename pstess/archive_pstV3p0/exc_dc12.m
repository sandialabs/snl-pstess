...
if flag == 0; % initialization
  if i ~= 0  % scalar computation
    ...
    V_R(i,1) = exc_con(i,10)*Efd(i,1) + exc_pot(i,1)*...
                   exp(exc_pot(i,2)*(Efd(i,1)))*Efd(i,1);
    if exc_con(i,1) == 1
      mult = 1;
    else
      mult = eterm(n,1);
 ...
 if i ~= 0 % scalar computation
    ...
    if exc_con(i,11) == 0 % no exciter dynamics
      ...
      SE = exc_pot(i,1)*exp(exc_pot(i,2)*abs(Efd(i,k)))...
               *sign(Efd(i,k));
      dEfd(i,k) = (V_R(i,k) - (exc_con(i,10)+SE)*Efd(i,k) )...
                  /exc_con(i,11);
    end
   ...
   if n_dc~=0
     ...
     if ~isempty(TE)
       ...
       dEfd(dc_idx(TE),k) = (V_R(dc_idx(TE),k) - (exc_con(dc_idx(TE),10) + SE)...
                            .*Efd(dc_idx(TE),k))./exc_con(dc_idx(TE),11);
     end
